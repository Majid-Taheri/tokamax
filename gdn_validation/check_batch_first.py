#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Do the batch-first input and output paths give the same gradients?
...
def run(bf_in, bf_out):
  ...
  outs = bwd._fused_dhu_wy_intra_cumsum_pallas_jit(
      ...
      chunk_size=BT, batch_first_in=bf_in, batch_first_out=bf_out, **kw)
  outs = list(outs)
  if bf_out:  # dq, dk, dv come back batch-first
    for i in range(3):
...
def main():
  ...
  ref = run(False, False)
  ok = True
  for label, bf_in, bf_out in (("inputs only", True, False),
                               ("inputs + outputs", True, True)):
    print(f"  --- {label} ---")
    got = run(bf_in, bf_out)
    ok = _cmp(ref, got) and ok
  return _lower(ok)
    ...
    try:
      jax.jit(
          lambda **kk: bwd._fused_dhu_wy_intra_cumsum_pallas_jit(
              ...
              per_channel_gate=False, chunk_size=BT, batch_first_in=True,
              batch_first_out=True, mini_batch=mb, **kk)
